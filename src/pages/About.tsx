...
const StackBadge: React.FC<{ label: string; description: string }> = ({ label, description }) => (
...
const About: React.FC = () => {
  ...
  const isItems: string[] = t("about.is_items", { returnObjects: true }) as string[];
  const isNotItems: string[] = t("about.is_not_items", { returnObjects: true }) as string[];
  ...
  return (
    <div className="py-6 max-w-2xl">
      ...
      <Section title={t("about.data_source_title")}>
        <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
          {t("about.data_source_text").split("CoinGecko")[0]}
          ...
          {t("about.data_source_text").split("CoinGecko")[1]}
        </p>
        <p className="text-sm text-gray-500 dark:text-gray-500 mt-2">
          {t("about.data_source_note")}
        </p>
      </Section>
      ...
      <Section title={t("about.privacy_title")}>
        <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
          {t("about.privacy_text")}
        </p>
      </Section>

      <Section title={t("about.author_title")}>
        <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
          {t("about.author_text")}
        </p>
        <a href="https://github.com/merlinfachetti/crypto-insight" target="_blank" rel="noopener noreferrer"
      ...
      <div className="pt-2">
        <Link to="/" className="inline-flex items-center gap-2 px-5 py-2.5 bg-blue-500 hover:bg-blue-600 text-white text-sm font-medium rounded-lg transition-colors">
          {t("about.back")}
        </Link>
      </div>
    </div>
